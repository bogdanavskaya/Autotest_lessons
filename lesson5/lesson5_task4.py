...
def scrabble(word):
    dict_of_words = dict.fromkeys(['а', 'в', 'е', 'ё', 'и', 'н', 'о', 'р', 'с', 'т'], 1)
    dict_of_words.update(dict.fromkeys(['д', 'к', 'л', 'м', 'п', 'у'], 2))
    dict_of_words.update(dict.fromkeys(['б', 'г', 'ь', 'я'], 3))
    dict_of_words.update(dict.fromkeys(['й', 'ы'], 4))
    dict_of_words.update(dict.fromkeys(['ж', 'з', 'х', 'ц', 'ч'], 5))
    dict_of_words.update(dict.fromkeys(['ф', 'ш', 'э', 'ю'], 8))
    dict_of_words.update(dict.fromkeys(['щ'], 10))
    dict_of_words.update(dict.fromkeys(['ъ'], 15))

    list_word = list(word)
    points = 0
    for let in list_word:
        points += dict_of_words[let]
    return points
